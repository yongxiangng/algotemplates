...
public:
    UnionFind(int N) {
        ...
        setSum.assign(N, 0); for (int i=0;i<N;i++) setSum[i] = i + 1;
    }
    ...
    int findSet(int i) { 
        ...
        int oldParent = p[i];
        int parent = findSet(p[i]);
        children[oldParent].erase(i);
        p[i] = parent;
    ...
    long sumOfSet(int i) { return setSum[findSet(i)]; }
    
    void move(int i, int j) {
        // move i to set containing j
        if (isSameSet(i, j)) return;                 // i and j are in same set
        if (sizeOfSet(i) == 1) {unionSet(i, j); return;}
        int x = findSet(i), y = findSet(j);          // find both rep items
        int newRoot = x == i ? *children[i].begin() : x;   // x == i => i is the root
        setSize[newRoot]=setSize[x];
        setSum[newRoot]=setSum[x];
        setSize[newRoot]--;
        setSize[y]++;
        setSum[newRoot]-=(i + 1);
        setSum[y]+=(i + 1);
        for (auto &child : children[i]) {
            p[child] = newRoot;
            if (child != newRoot) children[newRoot].insert(child);
        }
        if (x != i) {
            children[x].erase(i);
        }
