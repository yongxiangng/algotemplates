/**
 * https://github.com/stevenhalim/cpbook-code/blob/master/ch5/modInverse.cpp
 */
...
//}
//
//long modPow(long b, long p, long m) {                // assume 0 <= b < m
//    if (p == 0) return 1;
...
//}
//
//long extEuclid(long a, long b, long &x, long &y) {    // pass x and y by ref
//    long xx = y = 0;
//    long yy = x = 1;
//    while (b) {                                    // repeats until b == 0
//        long q = a/b;
...
//}
//
//long modInverse(long b, long m) {                   // returns b^(-1) (mod m)
//    long x, y;
//    long d = extEuclid(b, m, x, y);                 // to get b*x + m*y == d
//    if (d != 1) return -1;                         // to indicate failure
//    // b*x + m*y == 1, now apply (mod m) to get b*x == 1 (mod m)
//    return mod(x, m);
//}
