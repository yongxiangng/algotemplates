...
int main() {
    ...
    for (int i=0;i<m;i++) {
        for (int j=0;j<n;j++) {
            M.add_edge(i + 1, j + 1, (abs(courier[i].first - bottles[j].first) + abs(courier[i].second - bottles[j].second)
    ...
    for (int i=0;i<n-1;i++) {
        for (int j=0;j<n;j++) {
            M.add_edge(i + 1 + m, j + 1, 2 * (abs(rx - bottles[j].first) + abs(ry - bottles[j].second)));
        }
    }
