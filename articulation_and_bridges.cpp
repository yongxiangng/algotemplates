...
void articulationPointAndBridge(int u, vi &dfs_num, vi &dfs_low, vi &dfs_parent, vi &articulation_vertex,
                                int &dfsNumberCounter, int &dfsRoot, int &rootChildren, Al &al, unordered_set<ii, pair_hash> &bridges) {
    ...
    for (auto &v : al[u]) {
        if (dfs_num[v] == -1) {
            ...
            articulationPointAndBridge(v, dfs_num, dfs_low, dfs_parent, articulation_vertex,
                                       dfsNumberCounter, dfsRoot, rootChildren, al, bridges);
