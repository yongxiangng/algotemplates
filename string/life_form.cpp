...
unordered_map<int, int> idxToWord;
unordered_map<int, int> wordToLen;
unordered_map<int, int> wordStart;
int numWords;
...
public:
    ...
    void solve() {
//        printSA();
        int threshold = numWords / 2; // modify here
        int hi=n-1;
        int wordCnt = 0;
        int maxLen = 0;
        bool wasMax = false;
        unordered_map<int, int> freq; // freq of word
        vii ans;
        deque<ii> window;
        for (int lo=n-1;lo>=1;--lo) {
            int word = idxToWord[SA[lo]];
            freq[word]++;
            if (freq[word] == 1) wordCnt++; // new word

            while (!window.empty() && lo + 1 < n && window.back().first >= LCP[lo + 1]) {
                window.pop_back();                              // keep window sorted
            }

            if (lo + 1 < n) window.emplace_back(
                    min(wordToLen[idxToWord[SA[lo + 1]]] - 1 - (SA[lo+1] - wordStart[idxToWord[SA[lo+1]]]), LCP[lo+1]),
                    lo+1);

            // cout << lo << " " << hi << endl;

            if (wordCnt <= threshold) continue; // not yet hit half

            if (window.empty()) continue;

            while (freq[idxToWord[SA[hi]]] >= 2 || wordCnt - 1 > threshold) { // adjust window
                if (window.front().second == hi) window.pop_front();
                freq[idxToWord[SA[hi]]]--;
                if (freq[idxToWord[SA[hi]]] == 0) wordCnt--;
                --hi;
            }

            if (window.empty()) continue;

//             cout << lo << " " << hi << " " << window.front().first << endl;
            if (window.front().first > maxLen) {
                wasMax = false;
                ans.clear();
            }
            maxLen = max(maxLen, window.front().first);
            if (window.front().first == maxLen && !wasMax) {
                ans.push_back(window.front());
            }

            wasMax = window.front().first == maxLen;
        }
