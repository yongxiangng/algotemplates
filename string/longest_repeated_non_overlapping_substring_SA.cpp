...
public:
    ...
    bool possible(int len) {
        ...
        for (int i=0;i<n;i++) {
            ...
            minIdx = min(minIdx, SA[i]);
            maxIdx = max(maxIdx, SA[i]);
            if (maxIdx - minIdx >= len) return true;
        }
        return false;
    }

    void printSolutions(int len) {
        if (len == 0) return;
        ...
        for (int i=0;i<n;i++) {
            ...
            minIdx = min(minIdx, SA[i]);
            maxIdx = max(maxIdx, SA[i]);
            if (maxIdx - minIdx >= len) {
                if (!found) answers.insert(minIdx);
                found = true;
            }
        }
