...
struct vertex {
  ...
  int count;
  vector<vertex*> child;
  vertex(char a): alphabet(a), exist(false), count(0) { child.assign(26, NULL); }
};
...
public:
  ...
  void remove(string word, int idx) {
      vertex* cur = root;
      for (int i=0;i<=min((int) word.size() - 1, idx);i++) {
          int alphaNum = word[i]-'a';
          //cout << word[i] << endl;
          cur->count--;
          if (i == idx) cur->exist=false;
  ...
  void insert(string word) {                     // insert a word into trie
    ...
    for (int i = 0; i < (int)word.size(); ++i) { // O(n)
      ...
      cur->count++;
      if (cur->exist) { // there is some suffix that is a subset of curr word
          //cout << word << " " << i << endl;
          remove(word, i);
      }
      cur = cur->child[alphaNum];
  ...
  int getBest(vertex *v) {
      if (v == NULL) return 0;
      if (v->count <= 1) return 0;
      int best = 0;
      for (auto &c : v->child) {
          best = max(best, 1 + getBest(c));
      }
      return best;
  }
